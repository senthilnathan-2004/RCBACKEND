...
export const syncAdminUsers = async () => {
  try {
      ...
      try {
        ...
        if (user) {
          ...
          if (emailChanged) {
            ...
            if (emailExists) {
              logger.error(`Email ${normalizedEmail} is already taken by another user`)
              results.errors.push({
                role,
                email: normalizedEmail,
          ...
          // Update existing user with all fields from .env
          ...
          user.password = adminConfig.password // Will be hashed by pre-save hook
          user.hasChangedPassword = true

          // Save with validation
          await user.save({ validateBeforeSave: true })

          logger.info(`Updated admin user: ${role} (${normalizedEmail})${emailChanged ? ' - email changed' : ''}`)
          results.updated.push({ 
            role, 
            email: normalizedEmail,
            emailChanged: emailChanged ? user.email : undefined
          })
        } else {
          // Create new user
    ...
    const adminEmails = ADMIN_ROLES.map((role) => config.adminUsers[role]?.email?.toLowerCase()).filter(Boolean)
